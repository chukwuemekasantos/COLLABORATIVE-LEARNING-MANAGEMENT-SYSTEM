			<form id="signin_teacher" class="form-signin" method="post">
              ...
              <input type="file" class="input-block-level"  name="location"  required><br />
               <input type="text" class="input-block-level"  name="about" placeholder="About" required>
               <input type="text" class="input-block-level"  name="teacher_status" placeholder="teacher_status" required>
               <input type="text" class="input-block-level"  name="teacher_stat" placeholder="teacher_stat" required>
			...
			jQuery("#signin_teacher").submit(function(e){
					...
					$.ajax({
						...
						if(html=='true')
						{
						$.jGrowl("Welcome to Collaboative Learning Management System", { header: 'Sign up Success' });
						var delay = 1000;
							setTimeout(function(){ window.location = 'dasboard_teacher.php'  }, delay);  }else{
